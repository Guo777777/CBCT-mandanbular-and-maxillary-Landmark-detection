...
for patient_name in sorted(os.listdir(os.path.join(landmark_path))):
    ...
    for i, json_landmark_filename in enumerate(all_json_landmark_filename):
        ...
        if 'controlPoints' in markup and len(markup['controlPoints']) > 0:
            ...
            distance_golden.append(landmark_position)
        ...
        data_itkimage = sitk.ReadImage(os.path.join(tooth_region_path, number + '_' + name + '_pred'))
        ...
        for region_idx in sorted_top_regions:
            ...
            a1, b1, c1 = centroid
            centroid[2] = ((a1 / 128) * origin_crop_image_z_size + min_origin_tooth_z) * spacing[1] + origin[2]
            centroid[1] = ((b1 / 128) * origin_crop_image_x_size + min_origin_tooth_x) * spacing[0] + origin[1]
            centroid[0] = ((c1 / 128) * origin_crop_image_y_size + min_origin_tooth_y) * spacing[2] + origin[0]
    ...
    distance1 = []
    distance2 = []
    distance2.append(math.sqrt(
        (distance_golden[0][0] - distance_golden[1][0]) ** 2 + (distance_golden[0][1] - distance_golden[1][1]) ** 2 + (
                distance_golden[0][2] - distance_golden[1][2]) ** 2))
    distance2.append(math.sqrt(
        (distance_golden[0][0] - distance_golden[2][0]) ** 2 + (distance_golden[0][1] - distance_golden[2][1]) ** 2 + (
                distance_golden[0][2] - distance_golden[2][2]) ** 2))
    distance2.append(math.sqrt(
        (distance_golden[0][0] - distance_golden[3][0]) ** 2 + (distance_golden[0][1] - distance_golden[3][1]) ** 2 + (
                distance_golden[0][2] - distance_golden[3][2]) ** 2))
    distance2.append(math.sqrt(
        (distance_golden[2][0] - distance_golden[1][0]) ** 2 + (distance_golden[2][1] - distance_golden[1][1]) ** 2 + (
                distance_golden[2][2] - distance_golden[1][2]) ** 2))
    distance2.append(math.sqrt(
        (distance_golden[3][0] - distance_golden[1][0]) ** 2 + (distance_golden[3][1] - distance_golden[1][1]) ** 2 + (
                distance_golden[3][2] - distance_golden[1][2]) ** 2))
    distance2.append(math.sqrt(
        (distance_golden[2][0] - distance_golden[3][0]) ** 2 + (distance_golden[2][1] - distance_golden[3][1]) ** 2 + (
                distance_golden[2][2] - distance_golden[3][2]) ** 2))
    ...
    for i in distance1:
        for j in distance2:
            other.append([i, j, abs(i - j)])
        for k in other:
            if k[2] < min1[0][2]:
                min1[0] = [k[0], k[1], k[2]]
        min2.append(min1[0][0])
        min2.append(min1[0][1])
        other = []
        min1 = [[0, 0, 100]]
    for i in min2:
        data_list.append(i)
    ...
    # save to csv
# 创建 header 列表
header_list = ["case", "L1", "L1_pred", "L2", "L2_pred"]
# 以写方式打开文件。注意添加 newline=""，否则会在两行数据之间都插入一行空白。
with open("/home/user16/sharedata/GXE/SkullWidth/data/distance.csv", mode="w", encoding="utf-8-sig", newline="") as f:
